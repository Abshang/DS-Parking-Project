...

{
    ...
    licensePlates.insert(car->getId());
    return true;
}
// Add this method to ParkingManager.cpp:
bool ParkingManager::addCarToQueue(Car *car)
{
    if (!car || !entryQueue)
        return false;

    // Check for duplicate license plates
    if (licensePlates.find(car->getId()) != licensePlates.end())
    {
        std::cout << "Car " << car->getId() << " already in system (duplicate)\n";
        return false;
    }

    entryQueue->enqueue(car);
    licensePlates.insert(car->getId());
    std::cout << "Car " << car->getId() << " added to entry queue\n";
    return true;
}
